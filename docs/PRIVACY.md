...
| File | What it is |
|---|---|
| `node.key` | this device's cryptographic identity |
| `group.key` | the shared secret that defines your circle (established by pairing) |
| `atrest.key` | the key that encrypts your replica on disk |
| `doc.automerge` | your encrypted replica — memory, tabs, and file *offers* (not file bytes) |
| `blobs/` | content store for files you've shared or downloaded |
| `devices.json` | the devices you've linked (id + a friendly name you choose) |
| `offered_paths.json`, `downloads.json` | **local-only** maps of file id → path, so the app can "open location". Never synced. |

Your notes, tabs, and the file index are stored **encrypted at rest**
(XChaCha20-Poly1305). (Alpha caveat: the at-rest key is currently a `0600` file next to
the data — moving it into the OS keychain / a passphrase is planned. See
...
process on your machine, which reads/writes the same local replica. Whatever your AI does
with your memory/tabs/files is governed by that client's own data handling — Kith itself
sends nothing to a Kith server.
